...
typedef enum {
	PRG, FUNC, IMM, MUTE, ARR
} var_type;    //	program, function, immutable, mutable, array
...
int push_vs(VarStack *pvs, int *p_off, var_type t, char *s);
//	scan for first occurrence of ident in range, return its offset or -1
int search_vs(VarStack *pvs, int start, int end, char *s);
//	check over namespace for ident occurrence of different type, returns offset position or -1
int check_namespace(VarStack (*pvs), int end, char *s, var_type type);
//	check within a scope for ident occurrence of same type, returns offset position or -1
int check_scope(VarStack (*pvs), int start, int end, char *s, var_type type);
void semantic_dcl(int c, VarStack (*pvs), int offset, int *p_cur_offset, char *ident, var_type type);
void semantic_evk(int c, VarStack (*pvs), int offset, int cur_offset, char *ident, var_type type);
...
void semantic_dcl(int c, VarStack (*pvs), int offset, int *p_cur_offset, char *ident, var_type type) {
	if ( check_namespace(pvs, *p_cur_offset, ident, type) != -1 ) {
		printf("type of %s redeclared as %s at %d \n", ident, vt_name[type], c);
...
void semantic_evk(int c, VarStack (*pvs), int offset, int cur_offset, char *ident, var_type type) {
	if ( check_namespace(pvs, cur_offset, ident, type) != -1 ) {
		printf("type mismatch: cannot resolve %s %s at %d \n", vt_name[type], ident, c);
