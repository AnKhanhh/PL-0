...
void semantic_evk(int c, VarStack (*pvs), int offset, char *ident, Type type);
...
void semantic_evk(int c, VarStack (*pvs), int offset, char *ident, Type type) {
	if ( check_namespace(pvs, offset, ident, type) != -1 ) {
		printf("var_type mismatch: cannot resolve %s %s at %d \n", vt_name[type], ident, c);
	} else if ( check_scope(pvs, 0, offset, ident, type) == -1 ) {
