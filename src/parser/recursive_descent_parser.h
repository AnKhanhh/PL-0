...
void expression( FILE *in, Symbol *sb, int *tc, NodeAST *root );
void term( FILE *in, Symbol *sb, int *tc, NodeAST *root );
void factor( FILE *in, Symbol *sb, int *tc, NodeAST *root );
...
void statement( FILE *in, Symbol *sb, int *tc, NodeAST *root ) {
	if ( accept_tk( sb, IDENT )) {
		...
		if ( accept_tk( sb, ASSIGN )) {
			...
			expression( in, sb, tc, assignment );
...
//		prefix = CreateTreeNode( U_OP, ANN_TOKEN, &(sb->token));
//		InsertNode( root, prefix );
//		NextSymbol( in, sb, tc );
//	}
//	term( in, sb, tc, prefix );
//	while ( accept_tk( sb, PLUS ) || accept_tk( sb, MINUS )) {
//		NextSymbol( in, sb, tc );
//		term( in, sb, tc, root );
//	}
//}
